...
// Difficulty: MediumAccuracy: 42.73%Submissions: 255K+Points: 4
...
class Solution {
  public:
    int minCost(vector<int>& arr) {
        // code here
        //make a min heap then push all the elements 
        //pop the first two 
        //finally we get the two min sum 
        //poush that sum again 
        //then keep on doing till the size of the priority queue is 1
        priority_queue<int,vector<int>,greater<int>>pq;
        int totalSum=0;
        
        for(int i=0;i<arr.size();i++){
            //now just add the elements to the queue
            pq.push(arr[i]);
        }
        //now just the same procedure that i mentioned earlier
        while(pq.size()>1){
            int temp1=pq.top();
            pq.pop();
            int temp2=pq.top();
